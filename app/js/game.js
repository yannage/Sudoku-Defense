const Game = (function() {
    ...
    if (window.PixiRenderer && typeof PixiRenderer.init === 'function') {
        PixiRenderer.init(boardElement, cellSize);
    }
...
function reset() {
    ...
    function renderEnemies() {
        ...
        if (window.PixiRenderer && typeof PixiRenderer.syncEnemies === 'function') {
            PixiRenderer.syncEnemies(enemies);
        }
    }
...
function updateBoard() {
  ...
  for (let row = 0; row < 9; row++) {
    for (let col = 0; col < 9; col++) {
      ...
      if (isWavePhase) {
        ...
        if (tower) {
          // sprite handled by PixiRenderer
        }
  ...
  if (window.PixiRenderer && typeof PixiRenderer.syncTowers === 'function') {
    PixiRenderer.syncTowers(TowersModule.getTowers());
  }
