...
function createEnemy(type) {
  ...
  // Always use the first element of the path array which should be 
  // the leftmost entry point as generated by generateEnemyPath()
...
function damageEnemy(enemyId, damage) {
  ...
  // Check if enemy is defeated BEFORE rendering
  if (enemy.health <= 0) {
    // Immediately mark as inactive before defeat logic
    enemy.active = false;
    
    // Immediately remove from enemies array
...
function defeatEnemy(enemy) {
  // Ensure enemy is marked inactive before processing
  enemy.active = false;
  
  // Publish enemy defeated event BEFORE any potential rendering
  ...
  console.log(`Enemy ${enemy.id} defeated. Remaining: ${enemiesRemaining}`);
}
    /**
